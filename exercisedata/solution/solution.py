    ...
    if result.documents:
        for document in result.documents:
            if document.fields:
                ...
                if document.fields["PhoneNumber"]: print(f"PhoneNumber: {document.fields['State'].value_string}")
                if document.fields["DateOfSignature"]: print(f"DateOfSignature: {document.fields['DateOfSignature'].value_date}")
                if document.fields["ReceivedVaccineYes"]: print(f"ReceivedVaccineYes: {document.fields['ReceivedVaccineYes'].value_selection_mark}")
                if document.fields["ReceivedVaccineNo"]: print(f"ReceivedVaccineNo: {document.fields['ReceivedVaccineNo'].value_selection_mark}")
...
def analyze_document():
    ...
    poller = document_analysis_client.begin_analyze_document(
        "linkedinexercise02", #enter custom model id here
